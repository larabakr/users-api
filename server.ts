...
app.delete('/api/users/:id', (req, res) => {
    ...
    fs.writeFileSync('./data/users.json', JSON.stringify(users.filter((user: User) => user.id !== Number.parseInt(req.params.id))));
    res.redirect('/');
});
...
app.get('/api/users/:id', (req, res) => {
    ...
    res.json(users.filter((user: User) => user.id === Number.parseInt(req.params.id)));
});
